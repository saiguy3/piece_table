...
class PieceTable:
  ...
  def insert(self, text, index):
    ...
    # If insertion is at the end and the piece points to the end of the add buffer, just increase the length
    if (curr_piece.in_added and
        piece_offset == curr_piece.offset + curr_piece.length == added_offset):
      curr_piece.length += len(text)
      return

    # Split current piece into three separate pieces
    insert_pieces = [
      _Piece(curr_piece.in_added, curr_piece.offset, piece_offset - curr_piece.offset),
      _Piece(True, added_offset, len(text)),
      _Piece(curr_piece.in_added, piece_offset, curr_piece.length - (piece_offset - curr_piece.offset))
    ]
  ...
  def delete(self, index, length):
    ...
    self.pieces = self.replace_pieces(start_piece_index, delete_count, delete_pieces)
